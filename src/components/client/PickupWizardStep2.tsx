...
export const PickupWizardStep2: React.FC<PickupWizardStep2Props> = ({
  ...
  // Initialize boxes array based on boxCount with per-box electronics
  const [boxes, setBoxes] = useState<BoxItem[]>(() => {
    if (initialBoxes && initialBoxes.length > 0) {
      return initialBoxes.map((b, idx) => ({
        id: b.id || `box_${idx + 1}`,
        size: b.size || "Mediano",
        contentDescription: b.contentDescription || "",
        declaredValue: b.declaredValue ?? "",
        containElectronics: Boolean(b.containElectronics),
        electronicsDetails: b.electronicsDetails || "",
        electronicsDeclaredValue: b.electronicsDeclaredValue ?? "",
      }));
    }
  ...
  const triggerUpdate = (
    ...
    if (onUpdateData) {
      const anyElectronics = bList.some((b) => b.containElectronics);
      const combinedDetails = bList
        .filter((b) => b.containElectronics && b.electronicsDetails?.trim())
        .map((b) => `Caja #${bList.indexOf(b) + 1}: ${b.electronicsDetails?.trim()}`)
        .join(" | ");
      const combinedDeclaredVal = bList
        .filter((b) => b.containElectronics && typeof b.electronicsDeclaredValue === "number")
        .reduce((acc, b) => acc + (b.electronicsDeclaredValue as number), 0);

      onUpdateData({
        boxCount: bCount,
        boxes: bList,
        containElectronics: anyElectronics,
        electronicsDetails: combinedDetails,
        electronicsDeclaredValue: combinedDeclaredVal > 0 ? combinedDeclaredVal : "",
        containLithium: lith,
        notes: nts,
      });
    }
  ...
  const handleBoxCountChange = (newCount: number) => {
    ...
    triggerUpdate(val, updatedBoxes, hasLithium, notes);
  };
  ...
  const updateBoxItem = (index: number, field: keyof BoxItem, value: any) => {
    ...
    setBoxes(updated);
    triggerUpdate(boxCount, updated, hasLithium, notes);
  };

  const handleLithiumChange = (val: boolean) => {
    setHasLithium(val);
    triggerUpdate(boxCount, boxes, val, notes);
  };

  const handleNotesChange = (val: string) => {
    setNotes(val);
    triggerUpdate(boxCount, boxes, hasLithium, val);
  };
  ...
  const totalDeclaredBoxes = boxes.reduce(
    (acc, b) => acc + (typeof b.declaredValue === "number" ? b.declaredValue : 0),
    0
  );

  const boxesWithElectronicsCount = boxes.filter((b) => b.containElectronics).length;
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm space-y-6">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Indica cuántas cajas retirarás e ingresa los detalles obligatorios por cada caja (tamaño, contenido general, valor declarado y si contiene equipos electrónicos).
          </p>
        </div>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center border-2 border-slate-200 rounded-2xl p-1 bg-slate-50">
              <button
                ...
                className="w-10 h-10 rounded-xl bg-white border border-slate-200 text-slate-700 hover:bg-amber-500 hover:text-slate-950 hover:border-amber-500 flex items-center justify-center font-black transition-all cursor-pointer"
            ...
            <span className="text-xs text-slate-500 font-medium">
              Se desplegará una tarjeta por cada caja para detallar su contenido, valor y equipos electrónicos.
            </span>
          </div>
        </div>
            ...
            <div className="flex items-center gap-3 text-xs">
              {boxesWithElectronicsCount > 0 && (
                <span className="text-[11px] font-bold text-blue-700 bg-blue-50 px-2 py-0.5 rounded-md border border-blue-200 flex items-center gap-1">
                  ...
                  {boxesWithElectronicsCount} con electrónicos
                </span>
              )}
            ...
            {boxes.map((box, index) => (
              <div
                ...
                className={`p-5 rounded-2xl border transition-all space-y-4 relative ${
                  box.containElectronics
                    ? "bg-slate-50/90 border-blue-200 shadow-sm"
              ...
              >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {box.containElectronics && (
                      <span className="text-[10px] font-extrabold text-blue-700 bg-blue-100/80 px-2 py-0.5 rounded-full border border-blue-200 flex items-center gap-1">
                        <Laptop className="w-3 h-3 text-blue-600" /> Con Electrónicos
                      </span>
                    )}
                  </div>
                ...
                {/* 1. Selección de tamaño por caja */}
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                    {sizes.map((s) => (
                      <button
                        ...
                        className={`p-2.5 rounded-xl border-2 text-left transition-all flex items-center gap-2.5 cursor-pointer ${
                          box.size === s.id
                ...
                {/* 2 & 3. Campos obligatorios: Descripción y Monto declarado */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div className="md:col-span-2">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder="Ej. Ropa variada, calzado deportivo, sábanas, accesorios..."
                  ...
                  <div>
                    <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-600 mb-1">
                      3. Monto Declarado de la Caja ($ USD) *
                    </label>
                    <div className="relative">
                ...
                {/* 4. PREGUNTA DE EQUIPOS ELECTRÓNICOS POR CADA CAJA */}
                <div className="pt-3 border-t border-slate-200/80 space-y-3">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5">
                    <label className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                      <Laptop className="w-4 h-4 text-blue-600" />
                      4. ¿Esta caja contiene dispositivos o equipos electrónicos? *
                    </label>
                    <span className="text-[10px] text-slate-500">
                      Laptops, computadoras, pantallas, tabletas, consolas, teléfonos, etc.
                    </span>
                  </div>

                  <div className="flex items-center gap-3">
                    <label
                      className={`flex items-center gap-2 text-xs font-bold px-3.5 py-2 rounded-xl border cursor-pointer transition-all ${
                        box.containElectronics === true
                    ...
                    >
                      <input
                        type="radio"
                        name={`electronics_option_${box.id || index}`}
                        checked={box.containElectronics === true}
                        onChange={() => updateBoxItem(index, "containElectronics", true)}
                        className="text-blue-600 focus:ring-blue-500 w-3.5 h-3.5"
                      />
                      SÍ, contiene electrónicos
                    </label>

                    <label
                      className={`flex items-center gap-2 text-xs font-bold px-3.5 py-2 rounded-xl border cursor-pointer transition-all ${
                        box.containElectronics === false
                    ...
                    >
                      <input
                        type="radio"
                        name={`electronics_option_${box.id || index}`}
                        checked={box.containElectronics === false}
                        onChange={() => {
                          const updated = [...boxes];
                          updated[index] = {
                            ...updated[index],
                        ...
                        className="text-slate-600 focus:ring-slate-500 w-3.5 h-3.5"
                      />
                      NO contiene electrónicos
                    </label>
                  </div>

                  {/* Campos condicionales para electrónicos en ESTA caja */}
                  {box.containElectronics && (
                    <div className="p-3.5 rounded-xl bg-blue-50/70 border border-blue-200 space-y-3 animate-in fade-in">
                      <div className="flex items-center gap-1.5 text-blue-950 font-extrabold text-xs">
                        <Laptop className="w-3.5 h-3.5 text-blue-600" />
                        Detalle de Equipos Electrónicos en Caja #{index + 1}
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        <div className="md:col-span-2">
                          ...
                          <input
                            type="text"
                            required
                            value={box.electronicsDetails || ""}
                            onChange={(e) => updateBoxItem(index, "electronicsDetails", e.target.value)}
                        ...
                        <div>
                          ...
                          <div className="relative">
                            <DollarSign className="w-4 h-4 text-blue-500 absolute left-3 top-1/2 -translate-y-1/2" />
                            <input
                              type="number"
                              min="1"
                              step="1"
                              required
                              value={
                                box.electronicsDeclaredValue === "" || box.electronicsDeclaredValue === undefined
                                  ? ""
                                  : box.electronicsDeclaredValue
                              }
                              onChange={(e) => {
                                const val = e.target.value === "" ? "" : parseFloat(e.target.value);
                                updateBoxItem(index, "electronicsDeclaredValue", val);
                              }}
                              placeholder="Ej. 650"
        ...
        {/* SECCIÓN ADICIONAL: DECLARACIÓN SEPARADA DE BATERÍAS DE LITIO (REGULACIÓN AÉREA) */}
        <div
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="text-[11px] text-slate-500">
                  Baterías de Ion de Litio (Li-ion), baterías de polímero, powerbanks externos o pilas de litio contenidas en el envío.
                </span>
              </div>
            </div>
