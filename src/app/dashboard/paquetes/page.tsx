...
import { Search, Filter, Eye, Package, ShieldCheck, MapPin, CheckCircle2 } from "lucide-react";
...
interface PackageItem {
  ...
  weight: string;
  cubicFeet: string;
  status: "prealertado" | "recibido" | "transito" | "aduana" | "listo" | "entregado";
  statusLabel: string;
}
...
export default function MisPaquetesPage() {
  ...
  const packages: PackageItem[] = [
    ...
    },
    {
      ...
      description: "Lote de Repuestos Automotrices",
      origin: "Miami, USA",
      destination: "Santiago, CL",
      route: "MIA → SCL (Aéreo)",
      weight: "4.5 kg",
  ...
  const filteredPackages = packages.filter((pkg) => {
    ...
    return matchesSearch && pkg.status === activeTab;
  });
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-4">
          <div className="flex items-center gap-6 text-xs font-black uppercase tracking-wider text-slate-400 overflow-x-auto">
            {["todos", "recibido", "transito", "aduana", "listo", "entregado"].map((tab) => (
              <button
                key={tab}
        ...
        {/* Package Table Matching PDF Section 3.3 Specs */}
